...
// Client Functions
function genCalls<T extends ProcedureSet>(
...
const listener = serve(
  async (req: Request) => {
    try {
      ...
      console.error("Server Error", JSON.stringify(err), err);
...
console.log(await runRpc(rpcs, "ping"));
console.log(await runRpc(rpcs, "add", 8, 9));

// Common RPCs Definitions
export type MyRpcs = typeof rpcs;
export const myRpcNames = Object.keys(rpcs) as (keyof typeof rpcs)[];

const calls = genCalls<MyRpcs>(myRpcNames, "http://localhost:24515");
console.log(calls);
console.log("client call", await calls.add(8, 9));
